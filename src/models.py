...
class NMTModel(nn.Module):
    def __init__(self, hyp_params, input_modality=None, missing=None):
        """
        Construct a NMT model
        """
        ...
        self.embed_dropout = hyp_params.embed_dropout
        self.trans_dropout = hyp_params.trans_dropout
        self.modalities = input_modality  # the input modality
        self.missing = missing  # mark the missing modality
        ...
        # translate module
        self.encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=self.embed_dim,
                                                                        nhead=self.num_heads,
                                                                        dim_feedforward=self.embed_dim * 4),
                                             num_layers=self.trans_layers)
        self.decoder = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=self.embed_dim,
                                                                        nhead=self.num_heads,
                                                                        dim_feedforward=self.embed_dim * 4),
                                             num_layers=self.trans_layers)
    ...
    def forward(self, src, tgt, phase='train', eval_start=False):
        ...
        # Encoder
        if self.modalities == 'L':
            rep = self.encoder(x_l)[-1].unsqueeze(0)
        elif self.modalities == 'A':
            rep = self.encoder(x_a)[-1].unsqueeze(0)
        elif self.modalities == 'V':
            rep = self.encoder(x_v)[-1].unsqueeze(0)
        else:
            raise ValueError('Unknown modalities type')
        if phase != 'test':
            if self.missing == 'L':
                tgt = torch.cat((rep, x_l[:-1]), dim=0)
            elif self.missing == 'A':
                tgt = torch.cat((rep, x_a[:-1]), dim=0)
            elif self.missing == 'V':
                tgt = torch.cat((rep, x_v[:-1]), dim=0)
            else:
                raise ValueError('Unknown missing modality type')
        else:
            if eval_start:
                ...
                if self.missing == 'L':
                    tgt = torch.cat((rep, x_l), dim=0)
                elif self.missing == 'A':
                    tgt = torch.cat((rep, x_a), dim=0)
                elif self.missing == 'V':
                    tgt = torch.cat((rep, x_v), dim=0)
                else:
                    raise ValueError('Unknown missing modality type')
        #################################################################################
        # Decoder
        tgt_mask = self.generate_square_subsequent_mask(tgt.size(0))
        output = self.decoder(tgt, rep, tgt_mask=tgt_mask)
        output = self.out(output.transpose(0, 1))  # (batch, seq, embed_dim)
        return output
...
class FULLATTNModel(nn.Module):
    def __init__(self, hyp_params):
        """
        Construct a FullAttn model.
        """
        super(FULLATTNModel, self).__init__()
        if hyp_params.dataset == 'meld_senti' or hyp_params.dataset == 'meld_emo':
            self.orig_l_len, self.orig_a_len = hyp_params.l_len, hyp_params.a_len
            self.orig_d_l, self.orig_d_a = hyp_params.orig_d_l, hyp_params.orig_d_a
        else:
        ...
        # 3.2. FullAttn
        self.fullattn = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=self.embed_dim,
                                                                         nhead=self.num_heads,
                                                                         dim_feedforward=self.embed_dim * 4),
                                              num_layers=self.multimodal_layers)
    ...
    def forward(self, cls, x_l, x_a, x_v=None):
        """
        cls should have dimension [batch_size, seq_len, embed_dim]
        text, audio, and vision should have dimension [batch_size, seq_len, n_features]
        """
        if self.distribute:
            self.t.flatten_parameters()
